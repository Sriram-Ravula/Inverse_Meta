...
def get_dataset(args, config):
    ...
    if config.data.dataset == 'CIFAR10':
        ...
        if config.data.random_flip:
            dataset = CelebA(root=os.path.join(args.data_path, 'celeba'), split='train',
        ...
        test_dataset = CelebA(root=os.path.join(args.data_path, 'celeba_test'), split='test',
                              transform=transforms.Compose([
                                  transforms.CenterCrop(140),
                                  transforms.Resize(config.data.image_size),
                                  transforms.ToTensor(),
        ...
        if config.data.random_flip:
            dataset = FFHQ(path=os.path.join(args.data_path), transform=transforms.Compose([
                transforms.RandomHorizontalFlip(p=0.5),
                transforms.ToTensor()
            ]), resolution=config.data.image_size)
        else:
            dataset = FFHQ(path=os.path.join(args.data_path), transform=transforms.ToTensor(),
                           resolution=config.data.image_size)
