...
from utils.utils import dict2namespace, split_dataset, init_c, get_meta_optimizer, plot_images, get_measurement_images, MulticoilForwardMRI
from utils.loss_utils import get_A, get_measurements, get_likelihood_grad, get_meta_grad, get_loss_dict, get_mvue
...
class MetaLearner:
    ...
    def outer_step(self):
        ...
        for i, (sample, x_idx) in tqdm(enumerate(self.val_loader)):
            ...
            x_hat = SGLD_inverse(self.c, y, self.A, x_mod, self.model,
                                 self.sigmas, self.hparams,
                                 self.efficient_inp, maps)
            ...
            if self.hparams.outer.meta_type == 'maml':
                ...
                meta_grad += self.mle_step(x_hat, x, y, maps)
            else:
                raise NotImplementedError
    ...
    def mle_step(self, x_hat, x, y, s_maps=None):
        """
        ...
        cond_log_grad = get_likelihood_grad(self.c, y, self.A, x_hat, self.hparams, self.loss_scale, \
            efficient_inp=self.efficient_inp, retain_graph=True, create_graph=True, s_maps=s_maps)
    ...
    def val_or_test(self, validate):
        ...
        """
        if validate:
            cur_loader = self.train_loader
            iter_type = 'val'
        else:
            cur_loader = self.train_loader
            iter_type = 'test'

        for i, (sample, x_idx) in tqdm(enumerate(cur_loader)):
            if self.hparams.data.dataset == 'mri':
                x = sample['mvue'].type(torch.cuda.FloatTensor)
                mask = sample['mask'].to(self.hparams.device)
                maps = sample['maps'].to(self.hparams.device)
            ...
            x_hat = SGLD_inverse_eval(self.c, y, self.A, x_mod,
                                      self.model, self.sigmas,
                                      self.hparams,
                                      self.efficient_inp, maps)
